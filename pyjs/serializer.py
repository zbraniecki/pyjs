...
class Serializer(object):
    ...
    def is_bad_identifier(cls, e):
        return isinstance(e, pyjs.ast.Identifier) and \
                not re.match(r'^[_$A-Za-z][_$A-Za-z0-9]*$', e.name)
    ...
    def dump_for_head(cls, e, indent):
        ...
        if isinstance(e.left, pyjs.ast.VariableDeclaration):
            lhs = "%s %s" % (e.left.string,
                             cls.dump_declarators(e.left.declarations,
                                                  indent,
                                                  True))
        else:
    ...
    def dump_stmt(cls, e, indent=0):
        ...
        if isinstance(e, pyjs.ast.BlockStatement):
            ...
            return "%sreturn%s;\n" % \
                        (" " * indent,
                         " " + cls.dump_expr(e.argument,
                                             indent,
                                             0,
                                             False) if e.argument else "")
        elif isinstance(e, pyjs.ast.FunctionDeclaration):
            string = '%s%s%s' % (" " * indent,
                                 cls.dump_funcdecl("function",
                                                   e.id,
                                                   e,
                                                   indent),
    ...
    def dump_expr(cls, e, indent, cprec, noIn):
        if isinstance(e, pyjs.ast.ArrayExpression):
            return "[%s]" % \
                    ",".join([cls.dump_expr(i,
                                            indent,
                                            2,
                                            False) for i in e.elements])
            ...
            for i in range(0, len(p)):
                prop = p[i]
                if prop.kind == "init":
                    s.append("%s: %s" % (cls.dump_expr(prop.key,
                                                       indent,
                                                       18,
                                                       False),
                                         cls.dump_expr(prop.value,
                                                       indent,
                                                       2,
                                                       False)))
                elif prop.kind in ("get", "set"):
                    s.append(cls.dump_funcdecl(prop.kind,
                                               prop.key,
                                               prop.value,
                                               indent))
                else:
            ...
            return cls.wrap_expr("%s?%s:%s" % (
                                     cls.dump_expr(e.test,
                                                   indent,
                                                   4,
                                                   noIn),
                                     cls.dump_expr(e.consequent,
                                                   indent,
                                                   0,
                                                   noIn),
                                     cls.dump_expr(e.alternate,
                                                   indent,
                                                   3,
                                                   noIn)
                                 ),
            ...
            if len(e.arguments) == 0:
                ...
                string = "new %s%s" % (cls.dump_expr(e.callee,
                                                     indent,
                                                     18,
                                                     False),
